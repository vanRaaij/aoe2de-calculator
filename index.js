...
var $Skinney$murmur3$Murmur3$HashData = F4(
	function (shift, seed, hash, charsProcessed) {
		return {ai: charsProcessed, ao: hash, aa: seed, au: shift};
	});
...
var $Skinney$murmur3$Murmur3$finalize = function (data) {
	var acc = (!(!data.ao)) ? (data.aa ^ A2(
		$Skinney$murmur3$Murmur3$multiplyBy,
		$Skinney$murmur3$Murmur3$c2,
		A2(
			$Skinney$murmur3$Murmur3$rotlBy,
			15,
			A2($Skinney$murmur3$Murmur3$multiplyBy, $Skinney$murmur3$Murmur3$c1, data.ao)))) : data.aa;
	var h0 = acc ^ data.ai;
...
var $Skinney$murmur3$Murmur3$hashFold = F2(
	function (c, data) {
		...
		if (_v0 === 24) {
			...
			return {ai: data.ai + 1, ao: res, aa: data.aa, au: data.au + 8};
		}
	});
	...
	function (msg, civState) {
		...
		var newSelectedCiv = function () {
			if ($elm$core$String$isEmpty(selectedCiv)) {
				...
				if (!civ.$) {
					var c = civ.a;
					return $elm$core$Maybe$Just(c.Y);
				} else {
					return civState.aO;
				}
			}
		}();
		var adjustmentData = function () {
			if (!civ.$) {
				var c = civ.a;
				return c.S;
			} else {
				return $author$project$PageElements$Unit$baseUnitUpdateData;
			}
		}();
...
var $author$project$PageElements$Unit$getBaseUnitMax = F2(
	function (data, unitFamilyType) {
		if (!unitFamilyType) {
			return data.as;
		} else {
			return data.aw;
...
var $author$project$PageElements$Unit$update = F2(
	function (unitStates, _v0) {
		var baseUnits = _v0.S;
		var apply = function (unitState) {
...
var $author$project$Main$update = F2(
	function (msg, model) {
		var civMsg = msg;
		var _v1 = A2($author$project$PageElements$Civilization$update, civMsg, model.aE);
		var civState = _v1.a;
...
var $author$project$PageElements$Civilization$SelectCiv = $elm$core$Basics$identity;
...
var $author$project$PageElements$Civilization$toOption = function (s) {
	return A2(
...
var $author$project$PageElements$Civilization$civSelector = F2(
	function (p, options) {
		return A2(
				...
				[
					...
					_List_fromArray(
						[
							...
							$rtfeldman$elm_css$Html$Styled$Attributes$size(28),
							...
							$rtfeldman$elm_css$Html$Styled$Attributes$list('civs')
						]),
					_List_Nil),
					A2(
					...
					_List_fromArray(
						[
							$rtfeldman$elm_css$Html$Styled$Attributes$id('civs')
						]),
					A2($elm$core$List$map, $author$project$PageElements$Civilization$toOption, options))
				]));
	});
...
var $rtfeldman$elm_css$Css$inlineBlock = {g: 0, G: 'inline-block'};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$text('Civilisation')
					])),
				A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$rtfeldman$elm_css$Css$fontSize($rtfeldman$elm_css$Css$small)
							]))
					]),
				_List_fromArray(
					[
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A2($rtfeldman$elm_css$Html$Styled$Lazy$lazy, $author$project$PageElements$Civilization$show, model.aE))
			]));
};
